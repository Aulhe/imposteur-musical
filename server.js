...
app.get('/api/categories', (req, res) => {
  const cats = Object.entries(THEME_CATEGORIES).map(([id, cat]) => ({
    id,
    label: cat.label,
...
const THEME_CATEGORIES = {
  'lol-champions': { label: '🏆 Champions LoL', themes: [
    { normal: "Kai'Sa", impostor: 'Kassadin' },
    { normal: 'Yasuo', impostor: 'Yone' },
    { normal: 'Lux', impostor: 'Morgana' },
    { normal: 'Darius', impostor: 'Garen' },
    { normal: 'Thresh', impostor: 'Blitzcrank' },
    { normal: 'Jinx', impostor: 'Caitlyn' },
    { normal: 'Ahri', impostor: 'LeBlanc' },
    { normal: 'Lee Sin', impostor: 'Xin Zhao' },
    { normal: 'Zed', impostor: 'Talon' },
    { normal: 'Vayne', impostor: 'Lucian' },
    { normal: 'Teemo', impostor: 'Shaco' },
    { normal: 'Riven', impostor: 'Fiora' },
    { normal: 'Ezreal', impostor: 'Corki' },
    { normal: 'Vi', impostor: 'Ekko' },
    { normal: 'Katarina', impostor: 'Akali' },
    { normal: 'Senna', impostor: 'Lucian' },
    { normal: 'Warwick', impostor: 'Volibear' },
    { normal: 'Miss Fortune', impostor: 'Ashe' },
    { normal: 'Rengar', impostor: 'Kha\'Zix' },
    { normal: 'Sett', impostor: 'Darius' },
    { normal: 'Viego', impostor: 'Yone' },
    { normal: 'Sylas', impostor: 'Ekko' },
    { normal: 'Jhin', impostor: 'Aphelios' },
    { normal: 'Pyke', impostor: 'Thresh' },
    { normal: 'Sion', impostor: 'Cho\'Gath' },
    { normal: 'Nasus', impostor: 'Renekton' },
    { normal: 'Draven', impostor: 'Samira' },
    { normal: 'Twisted Fate', impostor: 'Graves' },
    { normal: 'Kayn', impostor: 'Zed' },
    { normal: 'Jax', impostor: 'Tryndamere' },
    { normal: 'Heimerdinger', impostor: 'Ziggs' },
    { normal: 'Morgana', impostor: 'Kayle' },
    { normal: 'Kindred', impostor: 'Lamb & Wolf' },
    { normal: 'Vex', impostor: 'Annie' },
    { normal: 'Bel\'Veth', impostor: 'Rek\'Sai' },
    { normal: 'Aatrox', impostor: 'Riven' },
    { normal: 'Orianna', impostor: 'Syndra' },
    { normal: 'Nautilus', impostor: 'Leona' },
    { normal: 'Evelynn', impostor: 'Elise' },
    { normal: 'Udyr', impostor: 'Hecarim' },
  ]},
  'lol-esport': { label: '🎮 Esport LoL', themes: [
    { normal: 'T1 (SKT)', impostor: 'Gen.G' },
    { normal: 'Faker', impostor: 'Chovy' },
    { normal: 'G2 Esports', impostor: 'Fnatic' },
    { normal: 'Cloud9', impostor: 'Team Liquid' },
    { normal: 'Worlds', impostor: 'MSI' },
    { normal: 'LCK (Corée)', impostor: 'LPL (Chine)' },
    { normal: 'LEC (Europe)', impostor: 'LCS (NA)' },
    { normal: 'Caps', impostor: 'Perkz' },
    { normal: 'Uzi', impostor: 'Deft' },
    { normal: 'ShowMaker', impostor: 'Faker' },
    { normal: 'Canyon', impostor: 'Jiejie' },
    { normal: 'Karmine Corp', impostor: 'Team Vitality' },
    { normal: 'Caedrel', impostor: 'LS' },
    { normal: 'DRX', impostor: 'DAMWON' },
    { normal: 'BLG', impostor: 'JDG' },
    { normal: 'Zeus', impostor: 'Kiin' },
    { normal: 'Gumayusi', impostor: 'Ruler' },
    { normal: 'Rekkles', impostor: 'Hans Sama' },
    { normal: 'TheShy', impostor: 'Nuguri' },
    { normal: 'Bengi', impostor: 'Blank' },
  ]},
  'gaming': { label: '🎮 Gaming', themes: [
  ...
  'genres-rap': { label: '🎤 Genres Rap', themes: [
  ...
  'anime': { label: '🍥 Anime / Manga', themes: [
  ...
  'films': { label: '🎬 Films / Séries', themes: [
  ...
  'internet': { label: '🌐 Culture Internet', themes: [
  ...
  'lifestyle': { label: '🛍️ Lifestyle / Marques', themes: [
  ...
  'culture': { label: '🌍 Culture Générale', themes: [
...
const ALL_CATEGORY_IDS = Object.keys(THEME_CATEGORIES);

function getThemesForCategories(categoryIds) {
  const ids = categoryIds.length > 0 ? categoryIds : ALL_CATEGORY_IDS;
  const themes = [];
  ids.forEach(id => {
    if (THEME_CATEGORIES[id]) themes.push(...THEME_CATEGORIES[id].themes);
  });
  return themes.length > 0 ? themes : getThemesForCategories(ALL_CATEGORY_IDS);
}
...
io.on('connection', (socket) => {
  ...
  socket.on('start-game', (code, settings, callback) => {
    ...
    lobby.selectedCategories = settings?.selectedCategories || [];
    lobby.round = 0;
...
function startNewRound(lobby) {
  ...
  let themes = getThemesForCategories(lobby.selectedCategories);
  if (lobby.customThemes.length > 0) themes = themes.concat(lobby.customThemes);
